...
module.exports.format = function(results) {
  stdout.write("\nCode Coverage Results:\n\n");
  return results.forEach(fileResultFormatter);
};

fileResultFormatter = function(result) {
  var coverage;
  coverage = colorize(result.coverage);
  return writeFileResult(coverage, result.executed, result.total, result.fileName);
};
